---
name: setup-pstack
description: "配置 pstack 各角色使用的模型与推理预算。检测你可用的模型,并写入一条始终生效的规则来覆盖技能默认值。当用户说 /setup-pstack、'configure pstack models'、'pstack budget'、『配置 pstack 模型』『改 pstack 用的模型』『调 pstack 推理预算』时使用。"
---
